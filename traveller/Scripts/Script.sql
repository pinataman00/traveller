...
-- myTrip 페이지에서 "삭제하기" 기능 구현하기
SELECT * FROM PLANNER;

DELETE FROM PLANNER WHERE PLANNER_NO = 'PLAN113';
SELECT * FROM PLAN WHERE PLANNER_NO = 'PLAN113';
SELECT * FROM PLAN;

-- 先 PLAN 삭제 -> 後 PLANNER삭제
DELETE FROM PLAN WHERE PLANNER_NO = 'PLAN113';
SELECT * FROM PLANNER WHERE PLANNER_NO = 'PLAN113';
DELETE FROM PLANNER WHERE PLANNER_NO = 'PLAN104';
--86
SELECT * FROM PLANNER WHERE PLANNER_NO = 'PLAN104';
SELECT * FROM PLAN WHERE PLANNER_NO = 'PLAN102';

DELETE FROM PLANNER WHERE PLANNER_NO='PLAN102'; 

SELECT * FROM MEMBER WHERE GRADE='planner';
...
SELECT * FROM REQUEST_PLAN;

SELECT * FROM REQUEST;

-- REQUEST 테이블 > 의뢰 승인-반려 여부 표시하기
ALTER TABLE REQUEST
ADD(APPROVAL VARCHAR2(100) DEFAULT 'N');

ALTER TABLE REQUEST DROP COLUMN APPROVAL;

CREATE SEQUENCE SEQ_REQUEST;

SELECT * FROM MEMBER;

--INSERT INTO MEMBER VALUES('testtest00','test@naver.com','1234','1234',default,'안녕',SYSDATE,null,null);
